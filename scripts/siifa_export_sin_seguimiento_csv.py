#!/usr/bin/env python3
"""
Exporta desde SIIFA las facturas SIN información de seguimiento (sin radicar).

Solo consulta la API SIIFA (GET /api/Factura?TieneRadicado=false).
No consulta ERP / PostgreSQL / SQL Server.

Uso:
  python scripts/siifa_export_sin_seguimiento_csv.py --nit-adquiriente 901543761
  python scripts/siifa_export_sin_seguimiento_csv.py --nit-adquiriente 901543761 --max-paginas 5
  python scripts/siifa_export_sin_seguimiento_csv.py --nit-adquiriente 901543761 -o export/siifa_facturas_sin_radicar.csv
  python scripts/siifa_export_sin_seguimiento_csv.py --nit-adquiriente 901543761 -o export/siifa_facturas_sin_radicar.csv --continuar
  python scripts/siifa_export_sin_seguimiento_csv.py --nit-adquiriente 901543761 --pausa-segundos 0.3
...
def fila_sin_seguimiento(raw: dict[str, Any], pagina: int) -> dict[str, Any]:
    ...
    return {
        ...
        "tiene_radicado_siifa": _first_value(raw, "tieneRadicado", "TieneRadicado") or "false",
    }
...
def export_sin_seguimiento_csv(
    *,
    cfg: Any,
    output: Path,
    ...
    try:
        for pagina, nuevas_rows, meta_parcial in yield_siifa_facturas_paginado(
            ...
            for raw in nuevas_rows:
                ...
                row = fila_sin_seguimiento(raw, pagina)
            ...
            save_export_progress(
                output,
                {
                    ...
                    "nit_adquiriente": nit,
                    "filtro_siifa": "TieneRadicado=false",
                    "registros_por_pagina": registros_por_pagina,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Exportar facturas SIIFA sin seguimiento/radicación (solo API SIIFA)."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Pausa entre páginas (p. ej. 0.3 para aliviar SIIFA).",
    )
    ...
    if args.verbose:
        print(
            f"siifa_export_sin_seguimiento_csv {SCRIPT_VERSION}\n"
            f"  script: {Path(__file__).resolve()}\n"
            f"  .env:   {env_file}\n"
            f"  filtro: TieneRadicado=false (sin seguimiento en SIIFA)",
            file=sys.stderr,
        )
    ...
    default_output = ROOT / "export" / f"siifa_sin_seguimiento_{stamp}.csv"
    output = args.output if args.output is not None else default_output
    if not output.is_absolute():
        output = (Path.cwd() / output).resolve()

    try:
        resumen = export_sin_seguimiento_csv(
            cfg=cfg,
            output=output,
